...
public class Main {
	...
	public static boolean exibirMenuAutenticacao(boolean keep, Scanner sc) {
		...
		case 1:
			...
			TipoConta contaSelecionadaLogin = (selecaoLogin == 1 ? TipoConta.USUARIO
					: (selecaoLogin == 2) ? TipoConta.PROPRIETARIO : null);
			if (contaSelecionadaLogin != null) {
				logarNaConta(sc, contaSelecionadaLogin);
			} else {
		...
		case 2:
			...
			TipoConta contaSelecionadaCadastro = (selecaoCadastro == 1 ? TipoConta.USUARIO
					: (selecaoCadastro == 2 ? TipoConta.PROPRIETARIO : null));
			if (contaSelecionadaCadastro != null) {
				cadastrarConta(sc, contaSelecionadaCadastro);
				System.out.print(
	...
	public static boolean exibirMenuLogadoUsuario(Usuario u, boolean keep, Scanner sc) {
		...
		System.out.print("Selecione a Opção desejada:\n" 
				...
				+ "2 - Visualizar imóveis do sistema.\n"
				+ "3 - Visualizar visitas agendadas.\n"
				+ "4 - Visualizar propostas realizadas.\n"
				+ "5 - Deslogar.\n"
				+ "6 - Sair do sistema\n");
		System.out.print("\tRESPOSTA: ");
		...
		case 2:
			// Mostrar feed de imóveis
			break;
		case 3:
			// Mostrar feed de visitas
			break;
		case 4:
			// Mostrar feed de propostas
			break;
		case 5:
			contaUsuario = null;
			break;
		...
		case 2:
			...
			case 1:
				...
				Casa casa = new Casa(endereco, tamanhoimovel, descricao, valorimovel, data, tipoDeVenda,
						contaProprietario, precoTerreno, casaUnicaTerreno);
	...
	public static void logarNaConta(Scanner sc, TipoConta contaSelecionada) {
		switch (contaSelecionada) {
		case USUARIO:
			if (listaDeUsuario.size() != 0) { // Verifica se existem usuários cadastrados no sistema
			...
			break;
		case PROPRIETARIO:
			if (listaDeProprietario.size() != 0) { // Verifica se existem proprietários cadastrados no sistema
				sc.nextLine();
				System.out.print("Digite o email cadastrado:\n");
				String email = sc.nextLine();
				for (Proprietario p : listaDeProprietario) {
					if (p.getEmail().equals(email)) {
						System.out.print("Digite a senha da sua conta:\n");
						String senha = sc.nextLine();
						if (p.getSenha().equals(senha)) {
				...
				System.out
						.println("Não há contas de proprietário cadastradas no sistema. Por favor, efetue o cadastro.");
			}
			break;
		}
		return;
